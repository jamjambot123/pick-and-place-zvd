...
uint32_t trapezoidalProfile(int32_t cs, int32_t ts, uint32_t baseIval) {
  ...
  // 가속 구간을 500ms(0.5초) 기준으로 동적 계산 (가감속을 길게 하여 부드럽게)
  int32_t dyn_accel = (int32_t)(500000.0f / ((MAX_STEP_INTERVAL_US + baseIval) / 2.0f));
  int32_t as = min(dyn_accel, ts / 2);
  int32_t ds = ts - as;
